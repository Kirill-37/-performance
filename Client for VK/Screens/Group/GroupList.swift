...
class GroupList: UITableViewController {
    ...
    func loadGroupsSections() {
        do {
            let realm = try Realm()
            //realm.refresh() не решает проблему загрузки экрана с первого запуска, все равно отображение групп идет только после второго запуска, мало того, переодически стало падать приложение:)
